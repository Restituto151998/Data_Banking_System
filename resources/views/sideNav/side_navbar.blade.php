                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                            <a class="dropdown-item text-center" style="color:#21791A;" href="/profile"
                             ...
                             <a class="dropdown-item text-center" style="color:#21791A;" href="{{ url()->previous() }}"
            ...
            <aside id="sidebar-wrapper">
                ...
                <ul class="sidebar-menu">
                    ...
                    @if (Auth::user()->type == 'ADMIN')
                        <li class="dropdown" >
                            <a href="/admin_dashboard" class="nav-link"  style="{{ request()->routeIs('admin.dashboard') ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="monitor"></i><span>{{ __('Dashboard') }}</span></a>
                        </li>
                    @endif
                    @if (Auth::user()->type == 'STAFF')
                        <li class="dropdown">
                            <a href="/staff_dashboard" class="nav-link " style="{{ request()->routeIs('staff.dashboard') ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="monitor"></i><span>{{ __('Dashboard') }}</span></a>
                        </li>
                    @endif
                    <li class="dropdown">
                        <a href="/profile" class="nav-link " style="{{ request()->routeIs('admin.profile') ? '  border-right: 10px solid green;' : '' }}"><i
                                data-feather="user"></i><span>{{ __('Profile') }} </span></a>
                    </li>
                    @if (Auth::user()->type == 'STAFF')
                        <li class="dropdown">
                            <a href="/resort_guest/{{ Auth::user()->resortList->resort_id }}" class="nav-link" style="{{ request()->routeIs('resorts.resort_guest.staff') ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="list"></i><span>{{ Auth::user()->resortList->resort_name }}</span></a>
                        </li>
                        <li class="dropdown">
                            <a href="/staff_register" class="nav-link" style="{{ request()->routeIs('staff.staff_register') ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="edit"></i><span>{{ __('Direct Register') }}</span></a>
                        </li>
                    @endif
                    @if (Auth::user()->type == 'ADMIN')
                        <li class="dropdown">
                            <a href="/add_resort" class="nav-link"  style="{{ request()->routeIs('admin.add_resort') ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="image"></i><span>{{ __('Add Resort') }}</span></a>
                        </li>

                        <li class="dropdown">
                            <a href="/add_user" class="nav-link"   style="{{ (request()->is('add_user*')) ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="users"></i><span>{{ __('Users') }}</span></a>
                        </li>

                        <li class="dropdown">
                            <a href="/resort_list" class="nav-link"  style="{{ (request()->is('resort_list*')) ? '  border-right: 10px solid green;' : '' }}"><i
                                    data-feather="list"></i><span>{{ __('Resort Assignee') }}</span></a>
                        </li>
                    @endif
                    <li class="dropdown">
                        <a href="/generate_qrcode" class="nav-link"  style="{{ request()->routeIs('resorts.generate_qrcode') ? '  border-right: 10px solid green;' : '' }}"><i
